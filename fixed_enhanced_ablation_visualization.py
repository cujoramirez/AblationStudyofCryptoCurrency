...
def create_radar_chart(df, output_dir):
    ...
    try:
        ...
        policy_instruments = df['Policy Instrument'].tolist()
          # Create a figure with multiple subplots - adjusted size for better layout
        fig, axes = plt.subplots(1, len(countries), figsize=(22, 10), 
        ...
        for i, country in enumerate(countries):
            ...
            policy_labels = policy_instruments + [policy_instruments[0]]
              # Plot the radar chart with thicker lines for better visibility - using colorblind-friendly colors
            # Use navy blue which is more colorblind-friendly than royalblue
            ax.plot(angles, values, linewidth=2.8, linestyle='solid', label=country, color='navy')
            ...
            for j, (angle, policy) in enumerate(zip(angles[:-1], policy_instruments)):
                ...
                if 0 <= angle_deg <= 60 or 300 <= angle_deg <= 360:
                    ha = 'left'
                    va = 'center'
                    offset = 1.8  # Increase offset for better readability
                    rotation = angle_deg
          ...
          # Adjust layout and spacing - bring charts closer together
        plt.tight_layout()
        plt.subplots_adjust(wspace=0.6)  # Reduce spacing between subplots to bring charts closer        # Add a common color bar with adjusted position to prevent overlap
        # Using a colorblind-friendly diverging palette (viridis-based)
        cbar_ax = fig.add_axes([0.92, 0.15, 0.02, 0.7])  # Adjust position to accommodate closer charts
